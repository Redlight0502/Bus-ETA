...
app.get('/api/stops', async (req, res, next) => {
  try {
    ...
    res.json({ data: stops.filter(s => !q || `${s.name_tc} ${s.name_en} ${s.stop}`.toLowerCase().includes(q)).slice(0, 60) });
...
app.use((error, req, res, next) => res.status(502).json({ error: 'Unable to reach the bus data provider. Please try again.' }));
